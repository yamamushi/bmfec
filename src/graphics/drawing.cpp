...
//
//  Modified from examples given at : http://members.chello.at/easyfilter/bresenham.html
//
    ...
    for (wd = (wd+1)/2; ; ) {                                   /* pixel loop */
        ...
        if (2*e2 >= -dx) {                                           /* x step */
            for (e2 += dy, y2 = y0; e2 < ed*wd && (y1 != y2 || dx > dy); e2 += dx)
                //setPixelColor(x0, y2 += sy, fmax(0,255*(abs(e2)/ed-wd+1)));
        ...
        if (2*e2 <= dy) {                                            /* y step */
            for (e2 = dx-e2; e2 < ed*wd && (x1 != x2 || dx < dy); e2 += dy)
                //setPixelColor(x2 += sx, y0, fmax(0,255*(abs(e2)/ed-wd+1)));
                if (y0 == y1) break;
            err += dx; y0 += sy;
        }
    }
